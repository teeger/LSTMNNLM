...
#include "nnlm/logbilinear_nnlm.h"
...
#define PROGRAM_NAME "Log-bilinear Neural Network Langauge Model"
...
int main(int argc, char **argv) {
  ...
  nnlm::LogBilinearNeuralNetLM logbilinear_nnlm;
  logbilinear_nnlm.set_debug(debug);
  logbilinear_nnlm.set_unk(unk);
  ...
  if (inmod != "") {
    ...
    logbilinear_nnlm.ReadLM(inmod);
    if (testfile != "") {
      logbilinear_nnlm.EvalLM(testfile, nce_ppl);
    }
    if (input_embedding != "") {
      logbilinear_nnlm.ExtractWordInputEmbedding(input_embedding);
    }
    if (output_embedding != "") {
      logbilinear_nnlm.ExtractWordOutputEmbedding(output_embedding);
    }
    return EXIT_SUCCESS;
  }
  ...
  logbilinear_nnlm.set_vocab_filename(vocfile);
  logbilinear_nnlm.set_train_filenames(trainfiles);
  logbilinear_nnlm.set_shuffle_datafiles(shuffle_datafiles);
  logbilinear_nnlm.set_shuffle_sentences(shuffle_sentences);
  logbilinear_nnlm.set_algopts(initalpha, batch, minimprovement);
  logbilinear_nnlm.set_l2_regularization_param(beta);
  logbilinear_nnlm.set_l2_regularization_param(0);
  logbilinear_nnlm.set_adagrad(adagrad);

  logbilinear_nnlm.set_nce(nce);
  logbilinear_nnlm.set_num_negative_samples(num_negative_samples);

  // must reset activations at the beginning of every sentence!
  logbilinear_nnlm.set_independent(true);
  logbilinear_nnlm.set_globalbias(globalbias);
  logbilinear_nnlm.set_bias(bias);
  logbilinear_nnlm.set_errorinput_cutoff(errorincutoff);

  logbilinear_nnlm.set_context_size(context_size);
  logbilinear_nnlm.set_nhiddens(hidden);

  logbilinear_nnlm.TrainLM(validationfile, outbase, nce_ppl);
}
