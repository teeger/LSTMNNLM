...
void LSTMRecurrentNeuralNetLM::ReadLMImpl(ifstream &ifs) {
  ...
  //TODO: architecture not fixed
  // connection_input_hidden_.ReadConnection(ifs);
  connection_input_mc_in_.ReadConnection(ifs);
  connection_input_mc_ig_.ReadConnection(ifs);
  connection_input_mc_og_.ReadConnection(ifs);
  connection_input_mc_fg_.ReadConnection(ifs);

  // connection_recurrenthidden_.ReadConnection(ifs);
  connection_recurrenthidden_mc_in_.ReadConnection(ifs);
  connection_recurrenthidden_mc_ig_.ReadConnection(ifs);
...
void LSTMRecurrentNeuralNetLM::WriteLMImpl(ofstream &ofs) {
  ...
  connection_input_mc_in_.WriteConnection(ofs);
  connection_input_mc_ig_.WriteConnection(ofs);
  connection_input_mc_og_.WriteConnection(ofs);
  connection_input_mc_fg_.WriteConnection(ofs);

  //connection_recurrenthidden_.WriteConnection(ofs);
  connection_recurrenthidden_mc_in_.WriteConnection(ofs);
...
void LSTMRecurrentNeuralNetLM::ExtractWordInputEmbeddingImpl(ofstream &ofs) {
  //const vector<vector<neuralnet::WeightType>> &weights = connection_input_hidden_.weights();
  // TODO
  const vector<vector<neuralnet::WeightType>> &weights = connection_input_mc_og_.weights();
  //if (connection_input_hidden_.storage_input_major()) {
  if (connection_input_mc_og_.storage_input_major()) {
...
void LSTMRecurrentNeuralNetLM::ExtractWordOutputEmbeddingImpl(ofstream &ofs) {
...
void LSTMRecurrentNeuralNetLM::PrintParamsImpl() {
  cout << "num_hiddens_: " << num_hiddens_ << endl;

  cout << "ngram_order_: " << ngram_order_ << endl;
  cout << "hash_table_size_: " << hash_table_size_ << endl;
  cout << "hash_mode_: " << hash_mode_ << endl;
  //lstm_cells_.PrintParams();
}

void LSTMRecurrentNeuralNetLM::CheckParamsImpl() {
...
void LSTMRecurrentNeuralNetLM::AllocateModel() {
  ...
  */
  connection_input_mc_in_.set_dims(true, vocab().size(), num_hiddens_);
  connection_input_mc_ig_.set_dims(true, vocab().size(), num_hiddens_);
  connection_input_mc_og_.set_dims(true, vocab().size(), num_hiddens_);
  connection_input_mc_fg_.set_dims(true, vocab().size(), num_hiddens_);
  connection_input_mc_in_.set_adagrad(adagrad());
  connection_input_mc_ig_.set_adagrad(adagrad());
  connection_input_mc_og_.set_adagrad(adagrad());
  connection_input_mc_fg_.set_adagrad(adagrad());
  connection_input_mc_in_.set_l2_regularization_param(l2_regularization_param());
  connection_input_mc_ig_.set_l2_regularization_param(l2_regularization_param());
  connection_input_mc_og_.set_l2_regularization_param(l2_regularization_param());
  connection_input_mc_fg_.set_l2_regularization_param(l2_regularization_param());
  last_connection_input_mc_in_.set_dims(true, vocab().size(), num_hiddens_);
  last_connection_input_mc_ig_.set_dims(true, vocab().size(), num_hiddens_);
  last_connection_input_mc_og_.set_dims(true, vocab().size(), num_hiddens_);
  last_connection_input_mc_fg_.set_dims(true, vocab().size(), num_hiddens_);
  last_connection_input_mc_in_.set_adagrad(adagrad());
  last_connection_input_mc_ig_.set_adagrad(adagrad());
  last_connection_input_mc_og_.set_adagrad(adagrad());
  last_connection_input_mc_fg_.set_adagrad(adagrad());
  last_connection_input_mc_in_.set_l2_regularization_param(l2_regularization_param());
  last_connection_input_mc_ig_.set_l2_regularization_param(l2_regularization_param());
  last_connection_input_mc_og_.set_l2_regularization_param(l2_regularization_param());
  last_connection_input_mc_fg_.set_l2_regularization_param(l2_regularization_param());

  connection_recurrenthidden_mc_in_.set_dims(true, num_hiddens_, num_hiddens_);
  connection_recurrenthidden_mc_ig_.set_dims(true, num_hiddens_, num_hiddens_);
  connection_recurrenthidden_mc_og_.set_dims(true, num_hiddens_, num_hiddens_);
  connection_recurrenthidden_mc_fg_.set_dims(true, num_hiddens_, num_hiddens_);
  connection_recurrenthidden_mc_in_.set_adagrad(adagrad());
  connection_recurrenthidden_mc_ig_.set_adagrad(adagrad());
  connection_recurrenthidden_mc_og_.set_adagrad(adagrad());
  connection_recurrenthidden_mc_fg_.set_adagrad(adagrad());
  connection_recurrenthidden_mc_in_.set_l2_regularization_param(l2_regularization_param());
  connection_recurrenthidden_mc_ig_.set_l2_regularization_param(l2_regularization_param());
  connection_recurrenthidden_mc_og_.set_l2_regularization_param(l2_regularization_param());
  connection_recurrenthidden_mc_fg_.set_l2_regularization_param(l2_regularization_param());
  last_connection_recurrenthidden_mc_in_.set_dims(true, num_hiddens_, num_hiddens_);
  last_connection_recurrenthidden_mc_ig_.set_dims(true, num_hiddens_, num_hiddens_);
  last_connection_recurrenthidden_mc_og_.set_dims(true, num_hiddens_, num_hiddens_);
  last_connection_recurrenthidden_mc_fg_.set_dims(true, num_hiddens_, num_hiddens_);
  last_connection_recurrenthidden_mc_in_.set_adagrad(adagrad());
  last_connection_recurrenthidden_mc_ig_.set_adagrad(adagrad());
  last_connection_recurrenthidden_mc_og_.set_adagrad(adagrad());
  last_connection_recurrenthidden_mc_fg_.set_adagrad(adagrad());
  last_connection_recurrenthidden_mc_in_.set_l2_regularization_param(l2_regularization_param());
  last_connection_recurrenthidden_mc_ig_.set_l2_regularization_param(l2_regularization_param());
...
void LSTMRecurrentNeuralNetLM::InitializeNeuralNet() {
  // Reset activations.
  ResetActivations();
  ...
  connection_input_mc_in_.RandomlyInitialize(rng_engine_);
  connection_input_mc_ig_.RandomlyInitialize(rng_engine_);
  connection_input_mc_og_.RandomlyInitialize(rng_engine_);
  connection_input_mc_fg_.RandomlyInitialize(rng_engine_);

  //connection_recurrenthidden_.RandomlyInitialize(rng_engine_);
  connection_recurrenthidden_mc_in_.RandomlyInitialize(rng_engine_);
  connection_recurrenthidden_mc_ig_.RandomlyInitialize(rng_engine_);
  connection_recurrenthidden_mc_og_.RandomlyInitialize(rng_engine_);
  connection_recurrenthidden_mc_fg_.RandomlyInitialize(rng_engine_);
...
void LSTMRecurrentNeuralNetLM::ResetActivations() {
...
void LSTMRecurrentNeuralNetLM::CacheCurrentParams() {
  //last_connection_input_hidden_ = connection_input_hidden_;
  last_connection_input_mc_in_ = connection_input_mc_in_;
  last_connection_input_mc_ig_ = connection_input_mc_ig_;
  last_connection_input_mc_og_ = connection_input_mc_og_;
  last_connection_input_mc_fg_ = connection_input_mc_fg_;

  //last_connection_recurrenthidden_ = connection_recurrenthidden_;
  last_connection_recurrenthidden_mc_in_ = connection_recurrenthidden_mc_in_;
  last_connection_recurrenthidden_mc_ig_ = connection_recurrenthidden_mc_ig_;
...
void LSTMRecurrentNeuralNetLM::RestoreLastParams() {

  //connection_input_hidden_ = last_connection_input_hidden_;
  connection_input_mc_in_ = last_connection_input_mc_in_;
  connection_input_mc_ig_ = last_connection_input_mc_ig_;
  connection_input_mc_og_ = last_connection_input_mc_og_;
  connection_input_mc_fg_ = last_connection_input_mc_fg_;

  //connection_recurrenthidden_ = last_connection_recurrenthidden_;
  connection_recurrenthidden_mc_in_ = last_connection_recurrenthidden_mc_in_;
...
void LSTMRecurrentNeuralNetLM::ForwardPropagate(size_t w) {  
  ...
  // propagate input -> cell and last_hidden -> cell 
  hidden_layers_.rotate(bptt_unfold_level_);
  //neuralnet::NeuralNetSigmoidLayer &current_hidden_layer = hidden_layers_[0];
  neuralnet::NeuralNetIdentityLayer &current_hidden_layer = hidden_layers_[0];
...
void LSTMRecurrentNeuralNetLM::BackPropagate(size_t w) {
  if (nce()) {
  ...
  //lstm_cells_.AccumulateGradients();
  // lstm cell accmulate gradient in backpropagate
...
void LSTMRecurrentNeuralNetLM::FastUpdateWeightsMajor(float learning_rate) {
  ...
  connection_recurrenthidden_mc_in_.FastUpdateWeightsMajor(learning_rate);
  connection_recurrenthidden_mc_ig_.FastUpdateWeightsMajor(learning_rate);
  connection_recurrenthidden_mc_og_.FastUpdateWeightsMajor(learning_rate);
  connection_recurrenthidden_mc_fg_.FastUpdateWeightsMajor(learning_rate);

  connection_input_mc_in_.FastUpdateWeightsMajor(learning_rate);
...
void LSTMRecurrentNeuralNetLM::FastUpdateWeightsMinor() {
  ...
  connection_recurrenthidden_mc_in_.FastUpdateWeightsMinor();
  connection_recurrenthidden_mc_ig_.FastUpdateWeightsMinor();
  connection_recurrenthidden_mc_og_.FastUpdateWeightsMinor();
  connection_recurrenthidden_mc_fg_.FastUpdateWeightsMinor();

  connection_input_mc_in_.FastUpdateWeightsMinor();
...
double LSTMRecurrentNeuralNetLM::GetLogProb(size_t w, bool nce_exact) {
  if (!nce()) {
    // Ignore nce_exact.
